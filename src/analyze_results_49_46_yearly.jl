...
# Congested
#conf_4 = JSON.parsefile(joinpath(results_folder,"Validation","Full_results","4_configurations_$(name_file)_24_8784.json"))
#conf_1 = JSON.parsefile(joinpath(results_folder,"Validation","Full_results","1_configurations_$(name_file)_24_8784.json"))
...
#=
function feasibility_check_bus_yearly(grid_original,grid_bs,switches_couples,extremes_ZILs,n_timesteps,opf_ac,result_bs,wind,load,load_multiplier)
    ...
    for t in 1:n_timesteps
        ...
        result_dict["$t"]["AC_OPF"]["primal_status"] = deepcopy(opf_ac["$t"]["termination_status"])
        ...
        _PMTA.prepare_AC_feasibility_check_AC_busbars(result_bs["$t"],test_case_bs_check_auxiliary,test_case_bs_check,switches_couples,extremes_ZILs,grid_original)
        ...
        result_dict["$t"]["$(conf)"]["AC_FC"] = Dict{String,Any}()
        result_dict["$t"]["$(conf)"]["AC_FC"]["objective"] = deepcopy(results_ac_check_single["objective"])
        result_dict["$t"]["$(conf)"]["AC_FC"]["primal_status"] = deepcopy(results_ac_check_single["termination_status"])
...
result_FC_and_OPF_comparison = feasibility_check_bus_yearly(test_case_original,test_case_updated_split,switches_couples,extremes_ZILs,hours,opf_yearly,bus_yearly,time_series.wind_cf_yearly,time_series.demand_yearly,load_multiplier)

json_result_FC_and_OPF_comparison = JSON.json(result_FC_and_OPF_comparison)
open(joinpath(results_folder,"result_FC_and_OPF_comparison_$(name_file).json"),"w") do f 
    write(f, json_result_FC_and_OPF_comparison) 
end 
=#
...
## Run this for standard
# -> with 49_46 standard, the most used topology is the original one so one needs to go from 2 to 5 (6?) best topology in that case
topologies_order = [2,3,4,6]
confs_selected = [parse(Int, sorted_confs[i][1]) for i in topologies_order]

## Run this for congested
...
opf_yearly_selection = JSON.parsefile(joinpath(results_folder_validation,"OPF_selection_$(name_file)_1_8784.json"))
number_topologies_yearly = count_topologies(hours,conf_and_timeseries,opf_yearly_selection,opf_yearly)
=#
#######################
...
number_topologies_1_yearly["OPF"]

#=
# For congested
count_ = 0
for i in 1:hours
    if (time_series.wind_cf_yearly[i] < 0.5 && time_series.demand_yearly[i] > 0.9)
        if i in timesteps_4
            count_ += 1
            println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
            push!(timesteps_1, i)
            filter!((x) -> x != i, timesteps_4)
        end
    end
    if (time_series.wind_cf_yearly[i] < 0.38 && time_series.demand_yearly[i] > 0.85) && !(i in timesteps_1)
        if i in timesteps_4
            count_ += 1
            println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
            push!(timesteps_1, i)
    ...
    if (time_series.wind_cf_yearly[i] < 0.25 && time_series.demand_yearly[i] > 0.85) && !(i in timesteps_1)
            if i in timesteps_4
                count_ += 1
                println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
                push!(timesteps_1, i)
    ...
    if (time_series.wind_cf_yearly[i] < 0.05 && time_series.demand_yearly[i] > 0.77) && !(i in timesteps_1)
        if i in timesteps_4
            count_ += 1
            println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
            push!(timesteps_1, i)
    ...
    if (time_series.wind_cf_yearly[i] < 0.075 && time_series.demand_yearly[i] > 0.78) && !(i in timesteps_1)
        if i in timesteps_4
            count_ += 1
            println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
            push!(timesteps_1, i)
    ...
    if (time_series.wind_cf_yearly[i] < 0.1 && time_series.demand_yearly[i] > 0.79) && !(i in timesteps_1)
        if i in timesteps_4
            count_ += 1
            println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
            push!(timesteps_1, i)
    ...
    if (time_series.wind_cf_yearly[i] < 0.12 && time_series.demand_yearly[i] > 0.8) && !(i in timesteps_1)
        if i in timesteps_4
            count_ += 1
            println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
            push!(timesteps_1, i)
    ...
    if (time_series.wind_cf_yearly[i] < 0.6 && time_series.demand_yearly[i] > 0.9) && !(i in timesteps_1)
        if i in timesteps_4
            count_ += 1
            println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
            push!(timesteps_1, i)
    ...
    if (time_series.wind_cf_yearly[i] < 0.25 && time_series.demand_yearly[i] > 0.83) && !(i in timesteps_1)
        if i in timesteps_4
            count_ += 1
            println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
            push!(timesteps_1, i)
    ...
    if (time_series.wind_cf_yearly[i] < 0.18 && time_series.demand_yearly[i] > 0.8) && !(i in timesteps_1)
        if i in timesteps_4
            count_ += 1
            println("$i: Topology 1 selected, wind cf = $(time_series.wind_cf_yearly[i]), load = $(time_series.demand_yearly[i])")
            push!(timesteps_1, i)
...
savefig(joinpath(results_figures_folder,"$(name_file)_distribution_$(n_timesteps)_timesteps_yearly.png"))
savefig(joinpath(results_figures_folder,"$(name_file)_distribution_$(n_timesteps)_timesteps_yearly.pdf"))
savefig(joinpath(results_figures_folder,"$(name_file)_distribution_$(n_timesteps)_timesteps_yearly.svg"))
    ...
    if t in 1:hours
        for (g_id,g) in test_case_updated_split["gen"]
            if haskey(g,"type") && g["type"] == "VOLL"
                shed_opf_hourly += opf_yearly["$t"]["on"]["gen"][g_id]["pg"]
            end
        end
...
load_original = sum(l["pd"] for (l_id,l) in test_case_original["load"] if parse(Int64,l_id) < 100)
load_total = sum(load_original*2*load[t] for t in 1:n_timesteps)

load_data_center = load_original + test_case_original["load"]["97"]["pd"]
load_data_center_total = sum(load_data_center*2*load[t] for t in 1:n_timesteps)
