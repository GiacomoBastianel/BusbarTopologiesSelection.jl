...
function batch_split(grid,first_hour,last_hour,size_batch,load_series,wind_series,multiplier_load,results_folder,simulation)
    for sample in Int64(first_hour/size_batch):Int64(last_hour/size_batch)
        ...
        json_opf = JSON.json(result)        
        open(joinpath(results_folder,"Result_BS_$(simulation)_hours_$(start_idx)_$(end_idx).json"),"w") do f 
            write(f, json_opf) 
        end
    end
end
...
function batch_opf(grid,first_hour,last_hour,size_batch,load_series,wind_series,multiplier_load,results_folder,simulation,formulation,optimizer)
    for sample in Int64(first_hour/size_batch):Int64(last_hour/size_batch)
        ...
        for h in start_idx:end_idx
            ...
            result_switches_lpac  = _PM.solve_opf(hourly_grid,formulation,optimizer)
            result_opf["$h"] = result_switches_lpac
        end
        json_opf = JSON.json(result_opf)        
        open(joinpath(results_folder,"Result_OPF_$(formulation)_$(simulation)_hours_$(start_idx)_$(end_idx).json"),"w") do f 
            write(f, json_opf) 
        end
    end
end
...
maximum(wind_69)
wind_cf = wind_69 ./ maximum(wind_69)
...
load = load_series_2 ./ maximum(load_series_2)
...
batch_split(test_case_updated_split,24,8784,24,load,wind_cf,1,results_folder,"Original")
batch_opf(test_case,24,8784,24,load,wind_cf,1,results_folder,"Original",LPACCPowerModel,gurobi)
batch_opf(test_case,24,8784,24,load,wind_cf,1,results_folder,"Original",ACPPowerModel,ipopt)

batch_split(test_case_updated_split,24,8784,24,load,wind_cf,2,results_folder,"Congested")
batch_opf(test_case,24,8784,24,load,wind_cf,2,results_folder,"Congested",LPACCPowerModel,gurobi)
batch_opf(test_case,24,8784,24,load,wind_cf,2,results_folder,"Congested",ACPPowerModel,ipopt)
...
batch_split(test_case_updated_split,24,8784,24,load,wind_cf,1,results_folder,"Original_49_46")
batch_split(test_case_updated_split,24,8784,24,load,wind_cf,2,results_folder,"Congested_49_46")
...
#confs_selected = [89,118,214,180] -> no data_center
...
open(joinpath(results_folder,"result_diff_top_data_center.json"),"w") do f 
...
open(joinpath(results_folder,"result_bs_data_center.json"),"w") do f 
    write(f, json_result_bs) 
end 

json_result_opf = JSON.json(result_opf)
open(joinpath(results_folder,"result_opf_data_center.json"),"w") do f 
    write(f, json_result_opf) 
end 

json_result_opf_ac = JSON.json(result_opf_ac)
open(joinpath(results_folder,"result_opf_ac_data_center.json"),"w") do f 
...
open(joinpath(results_folder,"result_diff_top_original_data_center.json"),"w") do f 
