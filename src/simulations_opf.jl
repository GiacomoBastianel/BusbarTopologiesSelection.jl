...
timeseries_selected = [88,3,33,27]
...
open(joinpath(results_folder,"result_diff_top.json"),"w") do f 
...
open(joinpath(results_folder,"result_bs.json"),"w") do f 
    write(f, json_result_bs) 
end 

json_result_opf = JSON.json(result_opf)
open(joinpath(results_folder,"result_opf.json"),"w") do f 
    write(f, json_result_opf) 
end 

json_result_opf_ac = JSON.json(result_opf_ac)
open(joinpath(results_folder,"result_opf_ac.json"),"w") do f 
...
confs_selected_original = [29,10,73,63]
for i in 1:n_timesteps
    sw_i = []
    for sw_id in 1:length(test_case_updated_split["switch"])
...
confs_selected_original = [29,10,73,63]
timeseries_selected_original = [3,4,2,1]
...
open(joinpath(results_folder,"result_diff_top_original.json"),"w") do f 
...
print_switch_configuration(10,confs_original,test_case_updated_split,test_case_original)
######################
# Printing the
...
print_switch_configuration(89 ,confs,test_case_updated_split,test_case_original)
print_switch_configuration(118,confs,test_case_updated_split,test_case_original)
print_switch_configuration(214,confs,test_case_updated_split,test_case_original)
print_switch_configuration(180,confs,test_case_updated_split,test_case_original)

print_switch_configuration(29,confs_original,test_case_updated_split,test_case_original)
