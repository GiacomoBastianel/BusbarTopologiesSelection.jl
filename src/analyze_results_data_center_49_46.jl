...
name_file = "49_46_standard_data_center"
#name_file = "49_46_congested_data_center"
...
n_confs_selected = 4
# -> with 49_46 standard, the most used topology is the original one so one needs to go from 2 to 5  best topology in that case
#topologies_order = [2,3,4,6]
confs_selected = [parse(Int, sorted_confs[i][1]) for i in 1+1:(n_confs_selected+1)]
...
open(joinpath(results_folder,"comparison_results_$(name_file)_2_5.json"),"w") do f 
...
result_congested_comparison = JSON.parsefile(joinpath(results_folder,"comparison_results_$(name_file)_2_5.json"))
...
#scatter(load_3, wind_3, label="Topology 1", color=:blue)
...
savefig(joinpath(results_figures_folder,"$(name_file)_distribution_$(n_timesteps)_timesteps.png"))
savefig(joinpath(results_figures_folder,"$(name_file)_distribution_$(n_timesteps)_timesteps.pdf"))
savefig(joinpath(results_figures_folder,"$(name_file)_distribution_$(n_timesteps)_timesteps.svg"))
